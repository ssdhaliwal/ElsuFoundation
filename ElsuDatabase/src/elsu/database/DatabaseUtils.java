...
public class DatabaseUtils {
	...
	*/
    ...
    // 20170205 added to differentiate from DML/DDL
    public static void executeDDL(Connection conn, String sql, 
            ArrayList<DatabaseParameter> params) throws SQLException,
            Exception {
        //Connection conn = this.getConnection();
        PreparedStatement stmt = null;
                ...
                switch (dbpm.getType()) {
                    case java.sql.Types.ARRAY:
                        if (dbpm.getValue() == null) {
                        	...
                        	// 20170124 - commented out due to conflict with DerbyDB
                        	// 20170208 - updated for depreciated ArrayDescriptor
                            Array ad = stmt.getConnection().createArrayOf("STR_ARRAY_TYP", (Object[])dbpm.getValue());
                        	stmt.setArray(paramIndex, ad);
                        }
                        break;
                    case java.sql.Types.DECIMAL:
                    case java.sql.Types.NUMERIC:
